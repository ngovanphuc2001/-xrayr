...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/ngovanphuc2001/xrayrr/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/ngovanphuc2001/xrayrr/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
    ...
    file="https://github.com/ngovanphuc2001/xrayr/raw/master/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "  Cài đặt mới, vui lòng tham khảo hướng dẫn trước：https://github.com/ngovanphuc2001/XrayR，Định cấu hình nội dung cần thiết"
    else
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/ngovanphuc2001/xrayr/master/XrayR.sh
